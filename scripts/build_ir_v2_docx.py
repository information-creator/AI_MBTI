"""AI-MBTI IR/PR v2 — 기업 대외 제출용 docx.

실행: python3 scripts/build_ir_v2_docx.py
출력: docs/IR_AI-MBTI_v2.docx
"""
...
PUBLIC = ROOT / "public"
OUT = ROOT / "docs" / "IR_AI-MBTI_v2.docx"
...
def build() -> None:
    ...
    # ══════════ 03. Traction ══════════
    section_header(doc, "03  TRACTION", "핵심 지표",
                   "실데이터 기준 — 2026년 3월 3일 ~ 4월 23일 (52일간).")

    add_kpi_row(doc, [
        ("누적 방문", "4,339", "▲ 52일 누적"),
        ("Meta Lead", "14", "세션당 1회 기준"),
        ("최우수 CPL", "₩3,980", "A/B 1위 (Social)"),
        ("전자책 수강", "88", "실질 학습"),
    ])

    doc.add_paragraph()
    add_h3(doc, "캠페인 성과 — 데이터 기반 소재 의사결정 (Meta Pixel Lead · 52일 누적)")
    add_table(doc,
              ["캠페인", "Lead", "CPL", "비고"],
              [
                  ["Social (최우수)", "7", "₩3,980", "예산 집중 배분 · CTR 3.66%"],
                  ["Simple", "6", "₩4,578", "성별 타겟팅 이원화 · 안정적"],
                  ["Fear (실험)", "1", "₩31,314", "감정 소구 한계 검증 → 조기 정지"],
                  ["합계 (활성 3개)", "14", "₩6,190", "평균 CPL · A/B 1위 Social"],
              ],
              highlight_rows=[0])

    doc.add_paragraph()
    add_h3(doc, "업계 평균 vs 당사 — Meta 광고 효율 벤치마크")
    add_table(doc,
              ["지표", "Meta 업계 평균", "당사 (Social 최우수)", "차이"],
              [
                  ["CTR", "1.91%", "3.66%", "▲ +92%"],
                  ["CPC", "₩1,100", "₩546", "▼ -50%"],
                  ["CPL", "약 ₩25,000", "₩3,980", "▼ -84%"],
              ])
    add_para(doc,
             "출처: WordStream Meta Ads Benchmark 2024 · 국내 교육/컨설팅 업종 기준. "
             "단일 소재 Social 기준 — 활성 3개 평균도 업계 대비 우수.",
             size=9, color=MUTED)

    doc.add_paragraph()
    add_pullquote(doc, "A/B 테스트로 최우수 소재 CPL을 업계 평균의 1/6 수준으로 압축했습니다.")
