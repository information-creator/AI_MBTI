...
export function calculateResult(answers: Record<number, string>): ResultData {
  ...
  let cScore = 0 // 독립성
  for (let i = 9; i <= 12; i++) if (answers[i] === 'Y') cScore++

  let dScore = 0 // 논리력
  for (let i = 13; i <= 16; i++) if (answers[i] === 'Y') dScore++

  let eScore = 0 // 실행력
  for (let i = 17; i <= 20; i++) if (answers[i] === 'Y') eScore++

  // 5차원 → 4차원 양극 변환 → 16유형 TypeCode
  const workStyle = cScore >= 3 ? 'H' : 'T'           // 독립성 → H/T
  const aiUsage = (aScore + bScore) >= 5 ? 'A' : 'S'  // AI 활용도+민감도 합산 → A/S
  const strength = dScore >= 3 ? 'L' : 'C'            // 논리력 → L/C
  const speed = eScore >= 3 ? 'F' : 'P'               // 실행력 → F/P
  ...
  return {
    typeCode,
    aiScore,
    scores: { a: aScore, b: bScore, c: cScore, d: dScore, e: eScore },
  }
}
